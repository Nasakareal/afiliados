...
class LonasExcelExporter
{
    ...
    public function create(Collection $lonas): string
    {
        ...
        try {
            ...
            $sheet->setCellValue('A1', 'Listado de lonas');
            ...
            foreach ($headers as $index => $header) {
                $sheet->setCellValueByColumnAndRow($index + 1, self::HEADER_ROW, $header);
            }
    ...
    private function applyStyles(Spreadsheet $spreadsheet, int $lastRow): void
    {
        ...
        $sheet->getStyle('A2:'.self::LAST_COLUMN.'2')->applyFromArray([
            'fill' => ['fillType' => Fill::FILL_SOLID, 'color' => ['rgb' => 'F4E6E9']],
            'font' => ['color' => ['rgb' => '5C0013'], 'italic' => true],
            'alignment' => ['vertical' => Alignment::VERTICAL_CENTER],
        ]);
        $sheet->getRowDimension(2)->setRowHeight(22);

        $headerRange = 'A'.self::HEADER_ROW.':'.self::LAST_COLUMN.self::HEADER_ROW;
        $sheet->getStyle($headerRange)->applyFromArray([
            'fill' => ['fillType' => Fill::FILL_SOLID, 'color' => ['rgb' => '5C0013']],
            ...
            'borders' => [
                'bottom' => ['borderStyle' => Border::BORDER_MEDIUM, 'color' => ['rgb' => '7A0019']],
            ],
        ]);
        $sheet->getRowDimension(self::HEADER_ROW)->setRowHeight(32);

        if ($lastRow >= self::FIRST_DATA_ROW) {
            $dataRange = 'A'.self::FIRST_DATA_ROW.':'.self::LAST_COLUMN.$lastRow;
            $sheet->getStyle($dataRange)->applyFromArray([
                'alignment' => ['vertical' => Alignment::VERTICAL_CENTER],
                'borders' => [
                    'bottom' => ['borderStyle' => Border::BORDER_HAIR, 'color' => ['rgb' => 'D9D9D9']],
                ],
            ]);
            $sheet->getStyle('D'.self::FIRST_DATA_ROW.':G'.$lastRow)->getAlignment()->setWrapText(true);
            $sheet->getStyle('J'.self::FIRST_DATA_ROW.':L'.$lastRow)->getAlignment()->setWrapText(true);
            $sheet->getStyle('H'.self::FIRST_DATA_ROW.':I'.$lastRow)->getNumberFormat()->setFormatCode('0.0000000');
            $sheet->getStyle('M'.self::FIRST_DATA_ROW.':N'.$lastRow)->getNumberFormat()->setFormatCode('#,##0');
            $sheet->getStyle('O'.self::FIRST_DATA_ROW.':P'.$lastRow)->getNumberFormat()->setFormatCode('dd/mm/yyyy hh:mm');
            $sheet->getStyle('J'.self::FIRST_DATA_ROW.':J'.$lastRow)->getFont()
        ...
        if ($lastRow >= self::FIRST_DATA_ROW) {
            $sheet->getStyle('A'.self::FIRST_DATA_ROW.':C'.$lastRow)->getAlignment()
                ->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle('F'.self::FIRST_DATA_ROW.':F'.$lastRow)->getAlignment()
                ->setHorizontal(Alignment::HORIZONTAL_CENTER);
        }
    }
}
